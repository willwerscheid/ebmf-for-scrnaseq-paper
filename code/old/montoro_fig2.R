...
cell_type <- sapply(strsplit(rownames(ebnmf_FF), "_"), `[[`, 5)
time_point <- sapply(strsplit(rownames(ebnmf_FF), "_"), `[[`, 2)
replicate <- sapply(strsplit(rownames(ebnmf_FF), "_"), `[[`, 3)
fluor <- sapply(strsplit(rownames(ebnmf_FF), "_"), `[[`, 4)

cell_type <- ifelse(
  cell_type %in% c("Basal"), paste0(cell_type, " (", time_point, ")"), cell_type
)
...
make_heatmap_tib <- function(FF, widths, sprs_thresh = 0.5, intercept = TRUE) {
  ...
  sorted_components <- tib |>
    group_by(Component, CellType) |>
  ...
  if (intercept) {
    sorted_components <- sorted_components |>
      arrange(desc(Component == "1"), CellType, desc(Loading))
  } else {
    sorted_components <- sorted_components |>
      arrange(CellType, desc(Loading))
  }
...
ebnmf_tib <- make_heatmap_tib(ebnmf_FF, ebnmf_widths, sprs_thresh = 0.27)
nnlm_tib <- make_heatmap_tib(nnlm_FF, nnlm_widths, sprs_thresh = 0.27)
